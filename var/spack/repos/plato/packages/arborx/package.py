...
class Arborx(CMakePackage):
    ...
    # provide them
    depends_on('kokkos@2.7.00:+cuda+cuda_lambda std=14', when='+cuda')
    depends_on('kokkos@2.7.00:+openmp std=14', when='+openmp')
    depends_on('kokkos@2.7.00:+serial std=14', when='+serial')
